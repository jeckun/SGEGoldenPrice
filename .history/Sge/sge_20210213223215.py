...
class Sge(Module):
    ...
    def export_json(self, **kwargs):
        ...
        rst = eg.session.query(Trade).filter(
            Trade.code == kwargs['code'])

        def convert_time(date):
            timeArray = time.strptime(date.strftime("%Y-%m-%d"), "%Y-%m-%d")
            return int(time.mktime(timeArray) * 100)

        exp = []
        for item in rst:
            print(item.trans_date, item.code, item.open,
                  item.high, item.low, item.close)
            exp.append([convert_time(item.trans_date), item.open,
                        item.high, item.low, item.close])
        filename = join('data', 'K_json.txt')
        save_to_json(exp)
