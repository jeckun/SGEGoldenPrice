    ...
    def analysis_table_data(self, day, row, col, **kwargs):
        ...
        try:
            ...
            if not rows:                                     # 处理 2016-12-16 模板改变的问题
                ...
                row = sge_xpath['row2']
                rows = self.parser.select(col)
                # 处理 2010-06-10 模板改变的问题
                if text(rows[0].text) != '合约' and day == '2010-06-10':
                    col = sge_xpath['col4']
                    row = sge_xpath['row4']
                    rows = self.parser.select(col)
            for i in range(1, len(rows)+1):
                ...
                if i == 1:
                    ...
                    if '加权平均' in fields:           # 2010-01-19 字段改名
                        fields[fields.index('加权平均')] = '加权平均价'
                    if '成交量(公斤）' in fields:
                        fields[fields.index('成交量(公斤）')] = '成交量'
                    if '成交额(元)' in fields:
                        fields[fields.index('成交额(元)')] = '成交金额'
                    if '成交金额（元）' in fields:
