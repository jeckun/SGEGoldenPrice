...
class Sge(Module):
    ...
    def run(self, robot: Robot, **kwargs):
        ...
        for task in robot.script:
            if task == 'get_catalog_list':
                ...
                kwargs.update(sge_xpath['col2'])
                kwargs.update(sge_xpath['row2'])
                self.get_days_data(
                    sge_xpath['row'], sge_xpath['col'], **kwargs)
    ...
    def get_days_data(self, row, col, **kwargs):
        # 获取每日行情各类合约交易数据
        for day in self.catalog_list:
            ...
            data = self.analysis_table_data(day[0], row, col, **kwargs)
    ...
    def analysis_table_data(self, day, row, col, **kwargs):
        ...
        if not rows:                                   # 2016-12-16 模板发生变化
            rows = self.parser.select(kwargs['col2'])
