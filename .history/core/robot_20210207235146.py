...
class Robot(object):
    ...
    def check(self):
        ...
        if self.get_state() == "闭市" and self.isSavetoDB == False:
            ...
            # 开始盯盘
            price = {}
            price["quote_time"] = self._spider.find_element_by_id(
                "quote_time").text

            while self._spider.find_element_by_id("status_em").text != "交易中":
                try:
                    if price["quote_time"] == self._spider.find_element_by_id("quote_time", text):
                        price.update({"name": self._spider.find_element_by_xpath(
                            '//*[@id="realtime_showname"]/span', text)})
                        price.update(
                            {"quote_time": self._spider.find_element_by_id("quote_time", text)})
                        price.update(
                            {"now_price": self._spider.find_element_by_id("now_price", text)})
                        price.update(
                            {"average_price": self._spider.find_element_by_xpath('//*[@id="tkChart_Hq"]/div[2]/span[4]', text)})
                        price.update(
                            {"upordown": self._spider.find_element_by_id("upOrDown_div", text)})
                        price.update(
                            {"buy_price": self._spider.find_element_by_id("buy_price", text)})
                        price.update(
                            {"sell_price": self._spider.find_element_by_id("sell_price", text)})
                        price.update(
                            {"open_price": self._spider.find_element_by_id("open_price", text)})
                        price.update(
                            {"close_price": self._spider.find_element_by_id("close_price", text)})
                        price.update(
                            {"high_price": self._spider.find_element_by_id("high_price", text)})
                        price.update(
                            {"low_price": self._spider.find_element_by_id("low_price", text)})

                        price.update({
                            "five_leval": self._spider.find_element_by_class_name("five_leval", text)
                        })
