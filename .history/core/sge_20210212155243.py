...
from lib.base import text, cache, load_by_json, save_to_json
from lib.os import exists, join, save_list_by_json, read_list_from_json
...
class Sge(Module):
    ...
    def get_days_data(self, row, col):
        ...
        for day in self.catalog_list:
            ...
            if exists(filename):
                self.trade_record.append(read_list_from_json(filename))
                continue
            # 没有下载过的交易数据，开始下载
